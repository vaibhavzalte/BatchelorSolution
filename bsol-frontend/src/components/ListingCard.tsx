...
function RoomVacancyCard({ vacancy, onMediaClick }: { vacancy: RoomVacancy; onMediaClick: (index: number) => void }) {
  ...
  return (
    <>
      <div className="relative h-52 bg-gray-100 flex gap-0.5 overflow-hidden rounded-2xl cursor-pointer pointer-events-auto" onClick={() => onMediaClick(0)}>
...
function RoomCard({ room, onMediaClick }: { room: Room; onMediaClick: (index: number) => void }) {
  const amenicons = [
  ...
  return (
    <>
      <div className="relative h-56 bg-gray-400 flex gap-0.5 rounded-2xl overflow-hidden cursor-pointer pointer-events-auto" onClick={() => onMediaClick(0)}>
        {displayImages.length > 0 ? (
          displayImages.map((img, i) => (
            <img
              key={i}
          ...
          <div className="flex items-center justify-between">
            ...
            {room.location && room.location.startsWith("http") && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 text-[10px] font-black text-indigo-600 hover:text-indigo-800 transition-colors uppercase tracking-widest pointer-events-auto"
              >
...
function MessCard({ mess, onMediaClick }: { mess: Mess; onMediaClick: (index: number) => void }) {
  ...
  const imageUrl = mess.images && mess.images.length > 0 ? normalizeUrl(mess.images[0]) : null;

  return (
    <>
      <div className="relative h-52 bg-gray-100 overflow-hidden rounded-2xl cursor-pointer pointer-events-auto" onClick={() => onMediaClick(0)}>
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={mess.title}
...
export default function ListingCard({ type, data }: ListingCardProps) {
  ...
  // Normalize all images and videos
      ...
      <div className="group relative bg-white p-2 rounded-[2rem] shadow-sm hover:shadow-2xl hover:shadow-indigo-500/10 border border-gray-100 overflow-hidden flex flex-col transition-all duration-500 hover:-translate-y-2 h-full">
        <div className="relative z-10 flex flex-col h-full">
          <div className="flex-1 flex flex-col">
            {type === "Room" && <RoomCard room={data as Room} onMediaClick={handleMediaClick} />}
            {type === "RoomVacancy" && <RoomVacancyCard vacancy={data as RoomVacancy} onMediaClick={handleMediaClick} />}
            {type === "Mess" && <MessCard mess={data as Mess} onMediaClick={handleMediaClick} />}
            {type === "FoodStall" && <FoodStallCard stall={data as FoodStall} onMediaClick={handleMediaClick} />}
          </div>
        </div>
      </div>
    </>
